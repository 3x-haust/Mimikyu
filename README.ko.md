...
캡처는 **전체 페이지 + motion 인지** 방식입니다: `prefers-reduced-motion: reduce`를
에뮬레이션하고, 페이지를 스크롤하며 `whileInView` 리빌을 트리거하고, 이미지를
`loading=eager`로 강제해서 decode까지 기다린 뒤 `fullPage`로 스크린샷을 찍습니다.
실제 사이트가 보이는 그대로 캡처합니다.
...
하나라도 실패하면 **완료가 아닙니다** — 고치고 다시 실행합니다. 한 가지 주의:
페이지가 **API 기반 콘텐츠**(말씀/갤러리/일정 등)를 렌더링하고 그것이 비어 있거나
스텁이거나 Figma placeholder와 다르면, 그 노드는 unmatched **경고**로만 표시되고
critical mismatch로 취급되지 않습니다. 해당 콘텐츠는 백엔드/픽스처를 띄워야
나타나므로, 데이터가 있어야만 나오는 콘텐츠를 픽셀 매칭하려고 무한 반복하지
마세요.
